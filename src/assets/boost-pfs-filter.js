...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  // Add Badge
  itemHtml = itemHtml.replace(/{{itemBadge}}/g, buildBadge(data));
  // Add Images
...
ProductGridItem.prototype.afterRender = function(data) {
  let productData = data;
  if (!productData) {
    productData = this.data;
  }
  const itemWrapper = jQ(this.$element);

  if (window.state.ProductBadges) {
    const badge = window.state.ProductBadges.find((badge) => productData.tags.includes(badge.tag));
    if (onSale) {
      itemWrapper.find('.boost-pfs-filter-product-item-badge-container').html(`
  ...
  const colorSwatchCount = itemWrapper.find('.boost-pfs-filter-item-swatch').attr('data-count');
  if (colorSwatchCount > 4) {
    itemWrapper.find('.boost-pfs-filter-item-swatch li').addClass('glide__slide');
    itemWrapper.find('.boost-pfs-filter-item-swatch').wrap('<div class="glide__track" data-glide-el="track"></div>');
    itemWrapper.find('.glide__track').wrap('<div class="glide" data-id="' + productData.id + '"></div>');

    itemWrapper.find('.glide').append(`
      <div class="glide__arrows" data-glide-el="controls">
        <button class="glide__arrow glide__arrow--left !hidden" data-glide-dir="<"></button>
  ...
  if (typeof productData.options_with_values != 'undefined' && productData.options_with_values.length > 0) {
    const swatchArr = productData.options_with_values.filter(function(option) {
      return option.name == 'size';
    });

    if (swatchArr.length > 0) {
      quickAddSizes = `<ul>`;
      const swachValues = swatchArr[0].values
      for (let i = 0; i < swachValues.length; i++) {
        quickAddSizes += `<li data-value="${swachValues[i].title}">${swachValues[i].title}</li>`;
      }
      quickAddSizes += `</ul>`;
    }
  }

  const colorSwatchArr = productData.options_with_values.filter(function(option) {
  ...
  itemWrapper.find('.boost-pfs-filter-product-item-image').append(quickAddHtml);

  // jQ(document).on('mouseleave', '.boost-pfs-filter-product-item-image', function(evt) {
  //   jQ(this).find('.boost-pfs-filter-product-item__quickAdd ul').removeClass('active').find('li').removeClass('active');
  // });

  itemWrapper.on('click', '.boost-pfs-filter-product-item__quickAdd button', function(evt) {
    const sizeOptions = jQ(this).attr('data-size-option');
    const colorOption = jQ(this).attr('data-color-option');

    if (sizeOptions != 'false') {
      jQ(this).find('ul').addClass('active');
    } else {
      const selectedVariant = productData.variants.filter(function(variant) {
        if (typeof variant.option_color !== 'undefined' && variant.option_color != colorOption) {
          return false;
        }
      ...
      if (selectedVariant.length > 0) {
        jQ(this).attr('data-variant', selectedVariant[0].id);
        quickAdd(selectedVariant[0].id);
      }
    }
  });

  itemWrapper.on('mouseenter', '.boost-pfs-filter-product-item__quickAdd button', function(evt) {
    jQ(this).find('ul').addClass('active');
  });

  itemWrapper.on('mouseleave', '.boost-pfs-filter-product-item__quickAdd button', function(evt) {
    jQ(this).find('ul').removeClass('active');
  });

  itemWrapper.on('mouseenter', '.boost-pfs-filter-product-item__quickAdd button li', function(evt) {
    evt.preventDefault();
    evt.stopImmediatePropagation();
    const buttonWrapper = jQ(this).closest('button');
    const colorOption = buttonWrapper.attr('data-color-option');
    const sizeOption = jQ(this).attr('data-value');
    ...
    const selectedVariant = productData.variants.filter(function(variant) {
      ...
      if (typeof variant.option_size !== 'undefined' && variant.option_size != sizeOption) {
        return false;
      }
    ...
    if (selectedVariant.length > 0) {
      buttonWrapper.attr('data-variant', selectedVariant[0].id);
    }

    jQ(this).addClass('active').siblings().removeClass('active');
  });

  itemWrapper.on('click', '.boost-pfs-filter-product-item__quickAdd button li', function(evt) {
    evt.preventDefault();
    evt.stopImmediatePropagation();

    const buttonWrapper = jQ(this).closest('button');
    const variantId = buttonWrapper.attr('data-variant');
  ...
  itemWrapper.find('.boost-pfs-filter-item-swatch li span').on('click', function(evt) {
    let swatchValue = jQ(this).attr('data-value');
    const productItemWrapper = jQ(this).closest('.boost-pfs-filter-product-item');
    let swatchImage = productItemWrapper.find('.boost-pfs-filter-product-item-main-image').attr('srcset');
    jQ(this).closest('li').addClass('active').siblings().removeClass('active');
    productItemWrapper.find('.boost-pfs-filter-item-swatch__label').attr('data-value', swatchValue).text(swatchValue);
    productItemWrapper.find('.boost-pfs-filter-product-item__quickAdd button').attr('data-color-option', swatchValue);
    productItemWrapper.find('.boost-pfs-filter-product-item-main-image').attr('src', swatchImage);
  });

  itemWrapper.on('mouseenter', '.boost-pfs-filter-item-swatch li span', function(evt) {
    let swatchValue = jQ(this).attr('data-value');
    const productItemWrapper = jQ(this).closest('.boost-pfs-filter-product-item');
    productItemWrapper.find('.boost-pfs-filter-item-swatch__label').text(swatchValue);
  });

  itemWrapper.on('mouseleave', '.boost-pfs-filter-item-swatch li span', function(evt) {
    const productItemWrapper = jQ(this).closest('.boost-pfs-filter-product-item');
    let swatchImage = productItemWrapper.find('.boost-pfs-filter-product-item-main-image').attr('src');
    let swatchValue = productItemWrapper.find('.boost-pfs-filter-item-swatch__label').attr('data-value');
    productItemWrapper.find('.boost-pfs-filter-item-swatch__label').text(swatchValue);
    productItemWrapper.find('.boost-pfs-filter-product-item-main-image').attr('srcset', swatchImage);
  });
}
...
ProductListItem.prototype.afterRender = function(data) {
  if (!data) data = this.data;
  const itemWrapper = jQ(this.$element);

  if (window.state.ProductBadges) {
    const badge = window.state.ProductBadges.find((badge) => data.tags.includes(badge.tag));
      ...
      for(var j=0; j < optionNames.length; j++){
        ...
        if (typeof data.options_with_values != 'undefined' && data.options_with_values.length > 0) {
          ...
          if (swatchArr.length > 0) {
            ...
            swatchesProductOptionHtml += '<div class="boost-pfs-filter-item-swatch__container"><ul data-count="'+ countSwatch + '" class="boost-pfs-filter-item-swatch boost-pfs-filter-item-swatch-option' + optionName + ' boost-pfs-filter-item-swatch-shape-' + swatchShape + ' boost-pfs-filter-item-swatch-type-' + swatchType + '">';
            ...
            for (var sIndex = 0; sIndex < countSwatch; sIndex++) {
              ...
              if (sImageIndex != '') {
                dataImgSrc = Utils.optimizeImage(data.images_info[sImageIndex - 1]['src'], dataImgSize + 'x') + ' ' + dataImgSize + 'w';
              }
              ...
              swatchesProductOptionHtml += `<li class="${sIndex == 0 ? 'active' : ''}">`;
                if (sIndex == 0 ) {
                  firstOptionName = swatchName;
                }
                if(swatchType == 'text'){
                  ...
                  */
                  swatchesProductOptionHtml += '<span tabindex="0" data-value="' + swatchName + '" aria-label="' + optionName + ': ' + swatchName + '" ' + 'style="background-color: ' + swatchName + '; ';
                  if (bgImageSrc != '') {
            ...
            swatchesProductOptionHtml += '</ul></div>';
            swatchesProductOptionHtml += '<div class="boost-pfs-filter-item-swatch__label" data-value="' + firstOptionName + '">'+ firstOptionName + '</div>';
            swatchesProductOptionHtml += `
...
function eventColorSwatches(event) {
  if (event == 'hover') {
    jQ(document).on('mouseenter', '.boost-pfs-filter-item-swatch li span', function() {
      var img = jQ(this).closest('.boost-pfs-filter-product-item-inner').find('.boost-pfs-filter-product-item-main-image');
  ...
  if (event == 'click') {
    jQ(document).on('click', '.boost-pfs-filter-item-swatch li span', function() {
      var img = jQ(this).closest('.boost-pfs-filter-product-item-inner').find('.boost-pfs-filter-product-item-main-image');
      var newImage = jQ(this).data('img');
      img.attr('srcset', newImage);
    });
  ...
  jQ(document).on('focus', '.boost-pfs-filter-item-swatch li span', function() {
    var img = jQ(this).closest('.boost-pfs-filter-product-item-inner').find('.boost-pfs-filter-product-item-main-image');
    var newImage = jQ(this).data('img');
    img.attr('srcset', newImage);
  });
...
Breadcrumb.prototype.afterRender = function(colData) {
}
...
Filter.prototype.afterRender = function(data) {
  ...
  const totalProducts = boostPFS.filter.filterResult.pagination.totalProduct;
  const currentProducts = boostPFS.filter.filterResult.pagination.toProduct;
  const currentPercentage = Math.ceil(currentProducts / totalProducts * 100);
  const loadMoreTotalHTML = `Showing <strong>${currentProducts}</strong> out of ${totalProducts} products`;
  const loadMoreProgresslHTML = `
  ...
  `;
  jQ('.boost-pfs-filter-load-more .boost-pfs-filter-load-more-total').html(loadMoreTotalHTML).after(loadMoreProgresslHTML);
  jQ('.boost-pfs-filter-load-more .boost-pfs-filter-load-more-button').html(`Load More`);
  jQ('.boost-pfs-filter-item-swatch__container .glide').each(function() {
    const glideId = jQ(this).attr('data-id');
    const slideCount = jQ('.glide[data-id="' + glideId + '"] .glide__slide').length;
    const glide = new Glide('.glide[data-id="' + glideId + '"]', {
      type: 'slider',
      perView: 4,
      bound: true,
    }).mount();
    glide.on('move.after', function(movement) {
      if (glide.index == 0) {
        jQ('.glide[data-id="' + glideId + '"]').find('.glide__arrow--left').addClass('!hidden');
      } else {
        jQ('.glide[data-id="' + glideId + '"]').find('.glide__arrow--left').removeClass('!hidden');
      }

      if (glide.index >= slideCount - 1) {
        jQ('.glide[data-id="' + glideId + '"]').find('.glide__arrow--right').addClass('!hidden');
      } else {
        jQ('.glide[data-id="' + glideId + '"]').find('.glide__arrow--right').removeClass('!hidden');
      }
    })
  });

  const searchResultHeaderText = jQ('.boost-pfs-search-result-header').text();
  jQ('.boost-pfs-search-result-header').text(searchResultHeaderText.replace(/{{ count }}/g, data.total_product));

  // No search results
  let noSearchResultContent = jQ('.product-list-no-search-result-text').html();
  ...
  `;
  jQ('.product-list-no-search-result-text').html(noSearchResultContent);
};
